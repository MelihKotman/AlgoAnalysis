# AlgoAnalysis

Bu repo, **Algoritma Analizi dersi** kapsamında hazırlanan algoritma uygulamalarını içerir.  
Projede hem **C** hem de **Python** kullanılmıştır. Her algoritma ayrı bir dosyada tutulur ve **tek bir main dosyası** üzerinden çalıştırılabilir.

---

## 📂 Proje Yapısı
### AlgoAnalysis-C  
C diliyle uygulanan algoritmalar aşağıdaki gibidir:
- Counting Sort
- Dynamic Programming
- Graphs Algorithms
- Heap Sort
- Insertion Sort
- Max Subarray Problem
- Merge Sort
- Min-Max Problem
- Quick Sort
- Randomized Select

### AlgoAnalysis-Python  
Python diliyle uygulanan algoritmalar aşağıdaki gibidir:
- Counting Sort
- Dynamic Programming
- Graphs Algorithms
- Heap Sort
- Insertion Sort
- Max Subarray Problem
- Merge Sort
- Quick Sort

---

> ℹ️ **Dipnot: Çalıştırma Talimatları**
> 
> **C tarafı:**  
> 1. CLion veya terminal ile `AlgoAnalysis-C` klasörü açılır.  
> 2. CMakeLists.txt build edilir.  
> 3. Terminalden çalıştırmak için:
> ```bash
> cd AlgoAnalysis-C
> cmake .
> make
> ./AlgoAnalysis
> ```
> 
> **Python tarafı:**  
> 1. Python 3 interpreter seçilir (PyCharm veya sistem Python).  
> 2. main.py çalıştırılır:
> ```bash
> cd AlgoAnalysis-Python
> python3 main.py
> ```
